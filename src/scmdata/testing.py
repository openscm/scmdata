...
def assert_scmdf_almost_equal(df1, df2, allow_unordered=False, check_index=True):
    # Check that the meta data is close

    if allow_unordered:
        pdt.assert_frame_equal(df1.meta, df2.meta, check_like=True, check_index=False)
