...
- Added {func}`scmdata.groupby.RunGroupby.apply_parallel` to perform parallelise large groupby operations ([#262](https://github.com/openscm/scmdata/pulls/262))
...
- Restored compatibility with pandas 3.0 and xarray 2025+ by patching four places where the older APIs are no longer accepted:
  ...
  ([#321](https://github.com/openscm/scmdata/pulls/321))
