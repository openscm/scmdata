...
# # Time operations
#
# Time operations are notoriously difficult. In this notebook we go through some of scmdata's time
...
# ## Data
#
# Here we use the RCP26 emissions data. This originally came from
...
# ## Resampling
#
# The first method to consider is `resample`. This allows us to resample a dataframe onto different
# timesteps. Below, we resample the data onto monthly timesteps.
...
# ## Interpolating
#
# Not all time points are supported by resampling. If we want to use custom time points (e.g. middle
# of year), we can do that with interpolate.
...
# %% [markdown]
# If we try to extrapolate beyond our source data but set `extrapolation_type=None`, we will receive
# an `InsufficientDataError`.
...
# %% [markdown]
# Generally the `interpolate` requires at minimum 3 times in order to perform any
# interpolation/extrapolation, otherwise an `InsufficientDataError` is raised. There is a special
# case where `constant` extrapolation can be used on a single time-step.
...
# ## Time means
#
# With monthly data, we can then take time means. Most of the time we just want to take the annual
# mean. This can be done as shown below.
...
# %% [markdown]
# As the data is an annual mean, we put it in July 1st (which is more or less the centre of the
# year).

# %% [markdown]
# ### Annual mean centred on January 1st
#
# Sometimes we want to take annual means centred on January 1st, rather than the middle of the year.
...
#
# Sometimes we want to take annual means centred on December 31st, rather than the middle of the
# year. This can be done as shown.
...
# %% [markdown]
# When the timeseries is particularly noisy, the different operations result in slightly different
# timeseries. For example, shifting to start of month smooths the data a bit (as you're
# interpolating and resampling the underlying data) while taking means centred on different points
# in time changes your mean as you take different windows of your monthly data.
...
# %% [markdown]
# The lines above don't match the underlying timeseries e.g. the monthly data minimum is in the
# wrong place.
