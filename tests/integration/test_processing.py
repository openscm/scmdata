...
@pytest.mark.parametrize("threshold,exp_vals", (
    (
        1.0,
        [
...
@pytest.mark.parametrize("return_year,conv_to_year", (
    (None, True),
    (True, True),
    (False, False),
))
def test_crossing_times(threshold, exp_vals, return_year, conv_to_year, test_processing_scm_df):
    ...
    if conv_to_year:
        exp_vals = [
            v if pd.isnull(v) else v.year
            for v in exp_vals
        ]

    exp = pd.Series(
        exp_vals,
        pd.MultiIndex.from_frame(test_processing_scm_df.meta),
    )
...
# calculate multiple stats (run multiple processing functions over, each with a name, return a dataframe)
