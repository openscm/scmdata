...
def test_xarray_plot_line(plumeplot_scmrun):
    ...
    plumeplot_scmrun["climate_model"] = plumeplot_scmrun.get_unique_meta("climate_model")[0]

    xr_ds = plumeplot_scmrun.to_xarray(dimensions=("scenario", "ensemble_member"))

    xr_ds["Surface Air Temperature Change"].median(dim="ensemble_member").plot.line(hue="scenario")
    xr_ds["Surface Air Temperature Change"].sel(scenario="a_scenario").plot.line(hue="ensemble_member")
